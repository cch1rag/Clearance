...
A native macOS privacy manager for reading and editing app permissions stored in Apple's TCC (Transparency, Consent, and Control) database. 100% local — no data leaves your machine.
...
- Runs entirely offline — no network requests, no telemetry, no account
...
2. Download `Clearance-1.0.0-universal.zip`
3. Unzip and drag `Clearance - App Permissions.app` to your `/Applications` folder
4. Right-click the app and choose **Open** the first time (required once for unsigned apps)
...
Clearance operates entirely on your local machine. It makes zero network requests with one exception: the first time you open a database, [sql.js](https://github.com/sql-js/sql.js) is fetched from cdnjs to parse the SQLite file in the browser layer. After that first load it is cached and works offline.

No analytics, no crash reporting, no cloud sync. The app never reads your actual TCC database automatically — you choose which file to open.
